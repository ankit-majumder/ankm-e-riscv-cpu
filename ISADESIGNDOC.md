...
4. B and J Instructions are the same as S and U instructions respectively, but implicitly shift immediate values right by 1, since instructions are 2 byte aligned in the instruction compression extension (however, for this processor they will 4 byte aligned in practice)
5. All branch immediates are sign extended
6. In practice, JALR rs1 will be x1 (Link Register), imm will be 0, and rd can be x0 (discard function pc) or general purpose (store function pc)
7. JALR clears the bit 0 of the pc to force function return to be 2-byte aligned
8. fence, ecall/ebreak, and csr instructions are stubbed and implemented as NOP, as they will not be called on a single cycle bare metal cpu with no operating system
9. For ecall and ebreak encoding, the funct7 columns are the hardcoded values of the immediates, not actual funct7 values.
10. All loads are sign extended, and all immediates are treated as signed values unless otherwise specified
11. Branches are grouped with ALU ops for subtraction, slt, and sltu and only taken if the controller branch_check flag is set
12. blt, bge are assigned to SLT ALU ops, and bltu and bgeu are assigned to SLTU ALU ops, because slt and sltu ops are implemented such that overflow edge cases are detected in Vivado synthesis so that branch overflows don't need to be calculated.
...
U: [31:12 => imm[19:0]][11:7 => rd][6:0 => opcode]  
...
|slti|I-Type|010|-------|0010011|slti rd, rs1, imm|rs1<imm?rd=1:rd=0, signed comparison|
|sltiu|I-Type|011|-------|0010011|sltiu rd, rs1, imm|rs1<imm?rd=1:rd=0, unsigned comparison|
...
|ecall|I-Type|000|000000000000|1110011|--------|Stubbed (Implemented as NOP)
...
# ALU Control Signals
|Alu_Code|Operation|Instructions|
|---|---|---|
|0000|ADD|add, addi, lw, lh, lb, lhu, lbu, sw, sh, sb, jalr, auipc, lui|
|0001|SUB|sub, bne, beq|
|0010|AND|and, andi|
|0011|OR|or, ori|
|0100|XOR|xor, xori|
|0101|SLL|sll, slli|
|0110|SRL|srl, srli|
|0111|SRA|sra, srai|
|1000|SLT|slt, slti, blt, bge|
|1001|SLTU|sltu, sltiu, bltu, bgeu|
